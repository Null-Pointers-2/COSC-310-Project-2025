...
def create_rating(rating_data: RatingCreate, current_user: dict = Depends(get_current_active_user), resources=Depends(get_resources)):
    """Rate a movie."""
    try:
...
def get_my_ratings(current_user: dict = Depends(get_current_active_user), resources=Depends(get_resources)):
    """Get current user's ratings."""
    ratings = ratings_service.get_user_ratings(resources, current_user["id"])
    return ratings

@router.get("/{rating_id}", response_model=Rating)
def get_rating(rating_id: int, current_user: dict = Depends(get_current_active_user), resources=Depends(get_resources)):
    """Get specific rating."""
...
def update_rating(rating_id: int, update_data: RatingUpdate, current_user: dict = Depends(get_current_active_user), resources=Depends(get_resources)):
    """Update a rating."""
...
def delete_rating(rating_id: int, current_user: dict = Depends(get_current_active_user), resources=Depends(get_resources)):
    """Delete a rating."""
    rating = ratings_service.get_rating_by_id(resources, rating_id)
    if not rating:
        raise HTTPException(status_code=404, detail="Rating not found")
