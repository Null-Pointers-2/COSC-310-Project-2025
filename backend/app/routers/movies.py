...
def get_movies(page: int = Query(1, ge=1, description="Page number"), page_size: int = Query(30, ge=1, le=100, description="Movies per page"), resources=Depends(get_resources)):
    """Get paginated list of movies."""
    return movies_service.get_movies(resources, page=page, page_size=page_size)

@router.get("/search", response_model=List[Movie])
def search_movies(query: str = Query(..., min_length=1), limit: int = Query(20, ge=1, le=100), resources=Depends(get_resources)):
    """Search movies by title."""
    return movies_service.search_movies(resources, query=query, limit=limit)

@router.get("/filter", response_model=List[Movie])
def filter_movies(genre: Optional[str] = None, limit: int = Query(20, ge=1, le=100), resources=Depends(get_resources)):
