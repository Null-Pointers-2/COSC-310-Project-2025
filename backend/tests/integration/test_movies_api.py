...
def test_get_movies_endpoint(mock_resources):
    mock_page = MoviePage(
        movies=[
            Movie(movieId=1, title="Test Movie", genres=["Action"]),
...
def test_get_movies_with_custom_pagination():
    mock_page = MoviePage(
        movies=[Movie(movieId=1, title="Test", genres=[])],
        total=100,
        page=3,
        page_size=10,
        total_pages=10,
...
def test_search_movies_endpoint():
    mock_movies = [
        Movie(movieId=1, title="Matrix", genres=["Action", "Sci-Fi"]),
        Movie(movieId=2, title="Matrix Reloaded", genres=["Action", "Sci-Fi"]),
    ]
...
def test_search_movies_with_limit():
    mock_movies = [Movie(movieId=i, title=f"Movie {i}", genres=[]) for i in range(5)]
...
def test_filter_movies_endpoint():
    mock_movies = [
        Movie(movieId=1, title="Action Movie 1", genres=["Action"]),
        Movie(movieId=2, title="Action Movie 2", genres=["Action"]),
    ]
...
def test_filter_movies_without_genre():
    mock_movies = [Movie(movieId=i, title=f"Movie {i}", genres=[]) for i in range(3)]
...
def test_get_movie_by_id_success():
    mock_movie = Movie(movieId=1, title="Test Movie", genres=["Action"], average_rating=4.5)
...
def test_get_movie_ratings_success():
    ...
    with (
        ...
        mock_get_movie.return_value = Movie(movieId=1, title="Test", genres=[])
