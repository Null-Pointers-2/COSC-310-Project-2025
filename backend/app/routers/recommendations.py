...
def get_my_recommendations(
    ...
    return recommendations_service.get_recommendations(
        resources, user_id=current_user["id"], limit=limit, force_refresh=force_refresh
    )
...
def get_similar_movies(movie_id: int, limit: int = Query(10, ge=1, le=50), resources=Depends(get_resources)):
