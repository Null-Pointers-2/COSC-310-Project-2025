...
class MoviesRepository:
    ...
    def __init__(self, movies_dir: str = "movies"):
        """
        Initialize with path to movies directory.
        Loads movie.csv and optionally genome data.
        """
    ...
    def get_paginated_movies(
        ...
        return paginated_df.to_dict(orient="records"), total # type: ignore[arg-type]
    ...
    def get_all(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        return sliced.to_dict(orient="records") # type: ignore[arg-type]
    ...
    def search(self, query: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        return results.to_dict(orient="records") # type: ignore[arg-type]
    ...
    def filter_by_year(self, year: int, limit: int = 20) -> List[Dict[str, Any]]:
        """Filter movies by year (extract from title if present, e.g. 'Toy Story (1995)')."""
        if self.movies_df is None or self.movies_df.empty:
            return []

        # Extract year from title, if it looks like "(1995)"
        df = self.movies_df.copy()
        df["year"] = df["title"].str.extract(r"\((\d{4})\)").astype(float)
        results = df[df["year"] == year].head(limit)
        return results.drop(columns="year").to_dict(orient="records") # type: ignore[arg-type]
