...
def get_all_users(current_admin: dict = Depends(get_current_admin_user)):
    ...
    return admin_service.get_all_users_with_stats()
...
def get_user_penalties(
    ...
    return admin_service.get_user_penalties(user_id)
...
def resolve_penalty(penalty_id: str, current_admin: dict = Depends(get_current_admin_user)):
    ...
    success = admin_service.resolve_penalty(penalty_id)
    if not success:
        raise HTTPException(
...
def delete_penalty(penalty_id: str, current_admin: dict = Depends(get_current_admin_user)):
    """Delete a penalty."""
    success = admin_service.delete_penalty(penalty_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Penalty {penalty_id} not found"
...
def check_user_violations(user_id: str, current_admin: dict = Depends(get_current_admin_user)):
    ...
    violations = admin_service.check_user_violations(user_id)
