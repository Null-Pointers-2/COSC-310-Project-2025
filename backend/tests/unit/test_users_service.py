...
@pytest.fixture
def sample_ratings():
    return [{"rating": 4.0}, {"rating": 5.0}, {"rating": 3.0}]


@pytest.fixture
def sample_penalties():
    return [{"reason": "Late submission", "status": "active"}]
...
def test_dashboard_success(mock_resources, sample_user, sample_ratings, sample_penalties):
    ...
    mock_resources.recommendations_repo.get_for_user.return_value = {
        "user_id": "user123",
        "recommendations": [{"movie_id": 1, "similarity_score": 0.9}],
    }
...
def test_returns_all_users(mock_resources, sample_user):
    mock_user_list = [
        sample_user,
        {**sample_user, "id": "user456", "username": "anotheruser"},
    ]
